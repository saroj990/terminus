...
export function formatPlanPrompt(plan: string[]): string {
  ...
  return `Planned steps:\n${lines.join("\n")}\nFollow this plan. Prefer search/read before write_file; run_workspace_tests after edits; deploy tools are dry-run unless LCA_DEPLOY_LIVE=1; retry once on failure.`;
}
